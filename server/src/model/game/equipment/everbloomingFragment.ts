...
import { Resurection } from "../status/resurection";

export class EverbloomingFragment extends Equipment {
  constructor() {
    super(
      "Everblooming Fragment",
      "A crystal shard formed from the heart of an eternal lotus. Warm to the touch with a chance of resurection."
    );
  }

  public equip(player: Player): void {
    player.getMonster()?.incCritRate(this.strength);
    player.addStatus(new Resurection())
  }
  ...
  public getStatDescription(): string {
    return "+" + this.strength + " Armour Class and 30% chance to resurrect with 50% HP";
  }
